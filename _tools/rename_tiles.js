...
for (
	let index = maxZoom, resolution = startResolution;
	index > minZoom;
...
try {
	fs.renameSync(
		`${imagesLocatedAt}${minZoom}${suffix}`,
		`${imagesLocatedAt}${minZoom}_0_0${suffix}`
	);
} catch (err) {}
...
function convert_names(prefix = "7_", resolution = 9216) {
	...
	for (let i = 0, row = 0; i < total_files; i++) {
		...
		let old_file_name =
			prefix + (i < 9 ? "0" + (i + 1) : "" + (i + 1)) + suffix;

		console.log(
			`${imagesLocatedAt}${old_file_name}`,
			`${imagesLocatedAt}${prefix}${column}_${row}${suffix}`
		);
		try {
			fs.renameSync(
				`${imagesLocatedAt}${old_file_name}`,
